...
function XAxis({
  ...
  const theme = useTheme()?.theme || { colors: { axis: { label: '#888', title: '#555', line: '#ddd' } } };
  ...
  const { angle, evenPositionsMap, formatLabel, rotate, textAnchor, tickValues } = useMemo(() => {
    ...
    const charLimit = forceFullLabels ? 100 : Math.min(MAX_LABEL_CHARS + 5, Math.max(8, maxCharsPerLabel));
  ...
  const renderAxisLabel = (
    ...
    const yOffset = showAxisLine ? labelOffset + (rotate ? ROTATED_LABEL_PADDING : 0) : labelOffset / 2;
  ...
  const mergedLabelProps = {
    ...
    dy: showAxisLine ? `${labelOffset + 4 + (rotate ? ROTATED_LABEL_PADDING : 0)}px` : `${labelOffset + 23}px`
  };
